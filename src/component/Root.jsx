...
const AuthRedirector = ({ children }) => {
  ...
  if (!app.user) {
    ...
    if (!pagesWithNoMfa.includes(location.pathname)) {
      ...
      let from = location;
      if (location.pathname === '/login' && location.state?.from) from = location.state?.from;
      return <Navigate to="/login/mfa" state={{ from }} replace />;
    }
  } else if (app.user.account_deleted) {
    if (location.pathname !== '/account_deleted') return <Navigate to="/account_deleted" replace />;
  } else {
