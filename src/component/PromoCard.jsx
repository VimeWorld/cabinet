      ...
      .then((body) => {
        if (body.success) {
          Notifications.success(
            <>
              Промо-код успешно активирован:{' '}
              {<span dangerouslySetInnerHTML={{ __html: body.response.info }} />}
            </>
          );
          reset();
        ...
        switch (body.response.type) {
          ...
          case 'not_exists':
            setError(
              'promo',
              { message: 'Такого промо-кода не существует' },
              { shouldFocus: true }
            );
            break;
          default:
        ...
        <form onSubmit={handleSubmit(activatePromo)}>
          <div className="d-flex">
            <Form.Group className="w-100" controlId="inp_promo">
              ...
              />
              {errors.promo && (
                <Form.Control.Feedback type="invalid">{errors.promo.message}</Form.Control.Feedback>
              )}
            </Form.Group>
            <div className="ms-3">
              <button
