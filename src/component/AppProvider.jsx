...
function AppProvider({ children }) {
  ...
  const updateApp = (newVal) => {
    setApp((old) => {
      ...
      if (old.savedTheme !== clone.savedTheme) {
        if (clone.savedTheme) localStorage.setItem('theme', clone.savedTheme);
      ...
      if (old.user !== clone.user) {
        ...
        if (!old.user && clone.user && !clone.skinModified)
          clone.skinModified = parseInt(
            sessionStorage.getItem('skin:' + clone.user.username) || '0'
          );
      }
  ...
  if (error) return <AuthLoadError error={error} app={value} />;
