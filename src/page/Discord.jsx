...
const CheckCode = ({ code, resetCode }) => {
    ...
    return <>
        ...
        {error && <>
            ...
            <button className="btn btn-ln btn-primary w-100" onClick={resetCode}>Попробовать снова</button>
        </>}
        {discordData && <>
            <p className="text-center">Аккаунты успешно связаны</p>
            <div className="text-primary fw-bold text-center">{app.user.username}</div>
            <div className="text-center"><i className="bi bi-arrow-down-up" /></div>
            <div className="text-primary fw-bold text-center">{discordData.username}#{discordData.discriminator}</div>
        </>}
    </>
}
...
const RequestPage = () => {
    return <>
        ...
        <a
            ...
            href={"https://discordapp.com/api/oauth2/authorize?" + new URLSearchParams({
