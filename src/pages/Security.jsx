      ...
      .then((body) => {
        ...
        switch (body.response.type) {
          case 'invalid_new_password':
            setError(
              'new_password',
              { message: 'Нельзя установить такой пароль' },
              { shouldFocus: true }
            );
            break;
          ...
          case 'passwords_equals':
            setError(
              'new_password',
              { message: 'Старый и новый пароли не отличаются' },
              { shouldFocus: true }
            );
            break;
          default:
          ...
          <Form.Group className="mb-3" controlId="new_password2">
            ...
            <Form.Control
              ...
              {...register('new_password2', {
                validate: (val) => {
                  if (watch('new_password') != val) return 'Пароли должны совпадать';
                }
              })}
  ...
  return (
    <div className="card">
      <div className="card-header">
        ...
        <span>
          Изменить Email можно только через <a href="https://vk.me/vimeworld">Поддержку</a>
        </span>
      </div>
      <div className="card-body">
...
const ModalSetupMfa = ({ show, close, onEnable }) => {
  ...
  const otpurl =
    'otpauth://totp/VimeWorld:' + app.user.username + '?issuer=VimeWorld&secret=' + secret;
  ...
  return (
    <Modal show={show} onHide={close} size="lg">
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Modal.Body>
          <p>
            Приложения для генерации кодов двухэтапной аутентификации позволяют получать коды даже
            без подключения к интернету. Воспользуйтесь любым приложением для генерации кодов
            двухэтапной аутентификации. Например, Google Authenticator для {iphone} или {android}.
          </p>
          <p>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Modal.Body>
          <Form.Group controlId="disable-totp-code">
            <Form.Label>
              Для отключения двухэтапной аутентификации введите одноразовый код из приложения
            </Form.Label>
            <Form.Control
              {...register('code', {
                required: true,
...
const MfaCard = () => {
  ...
  return (
    <div
      ...
      <div className="card-body">
        <p className="">
          Помимо пароля, для входа также будет требоваться одноразовый код из приложения.
        </p>
          ...
          <>
            <p className="mb-0 d-flex justify-content-between align-items-center">
              <span className="text-danger">
                <i className="bi bi-x-lg" /> Отключена
              </span>
              <button className="btn btn-primary" onClick={() => setModalSetupMfa(true)}>
                Включить
              </button>
              <ModalSetupMfa
                show={modalSetupMfa}
        ...
        {app.user.mfa == 'completed' && (
          <div className="d-flex justify-content-between align-items-center">
            <span className="text-success">
              <i className="bi bi-check-lg" /> Активна
            </span>
            <button className="btn btn-outline-danger" onClick={() => setModalDisableMfa(true)}>
              Отключить
            </button>
            <ModalDisableMfa
...
const ModalAccountDelete = ({ show, close }) => {
  ...
  return (
    <Modal show={show} onHide={close}>
      ...
      <Modal.Body>
        <div className="mb-3">
          Введите свой ник <b>{app.user.username}</b> в поле ниже:
        </div>
        <input
  ...
  return (
    <div className="card border border-danger">
      ...
      <div className="card-body text-center">
        ...
        <ModalAccountDelete show={modalDisableMfa} close={() => setModalDisableMfa(false)} />
      </div>
    </div>
  );
};
