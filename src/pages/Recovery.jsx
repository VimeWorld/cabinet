      ...
      .then((body) => {
        if (body.success) {
          Notifications.success(
            'Ваш пароль изменен. Теперь вы можете использовать его для входа в аккаунт.'
          );
...
const RecoveryPage = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        Notifications.success('Письмо с подтверждением было отправлено на вашу почту', {
          ttl: 30000
        });
      } else {
        switch (body.response.type) {
  ...
  return (
    <OuterPage>
      <Form onSubmit={submit}>
        ...
        <p>
          Вам будет выслано письмо с подтверждением, которое будет действительно в течение одного
          часа.
        </p>
