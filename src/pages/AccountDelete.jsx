...
const DeleteInProgressFragment = ({ progress }) => {
  ...
  if (!progress.cancel_possible)
    return <div>Аккаунт будет окончательно удален в ближайшее время.</div>;

  return (
    <div>
      <p>
        Аккаунт будет окончательно удален после{' '}
        <b>{new Date(Date.parse(progress.cancel_deadline)).toLocaleString()}</b>. До этого времени
        вы можете отменить удаление.
      </p>
...
const DeleteConfirmFragment = ({ token }) => {
  ...
  return (
    <>
      <p>
        После нажатия кнопки <b>Удалить аккаунт</b> у вас будет еще <b>20 дней</b> чтобы передумать
        и восстановить его. До этого времени вы можете отменить удаление.
      </p>

      <p className="text-danger">
        Ваш ник не будет доступен для регистрации еще <b>3 месяца</b> после удаления аккаунта.
      </p>
...
const AccountDeletedStatePage = () => {
  ...
  if (!app.user.account_deleted) return <></>;
  ...
  return (
    <OuterPage>
      ...
      <button className="btn btn-link mt-3" onClick={logout}>
        Выход
      </button>
    </OuterPage>
  );
