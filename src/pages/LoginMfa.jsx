      ...
      if (response.ok) {
        ...
        switch (body.response.type) {
          case 'invalid_code':
            setError(
              'code',
              { type: 'custom', message: 'Введен неправильный код' },
              { shouldFocus: true }
            );
            break;
          case 'throttle':
            ...
            if (retryAfter) {
              ...
              Notifications.warning(
                'Слишком много попыток, повторите снова через ' +
                  ruPluralize(minutes, ['минуту', 'минуты', 'минут'])
              );
            } else {
    ...
    <OuterPage>
      <Form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
        ...
        <Form.Group className="mb-3" controlId="code">
          <Form.Label>
            Вам необходимо ввести один из кодов восстановления, которые вы получили при включении
