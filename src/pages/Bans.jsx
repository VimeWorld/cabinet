...
const ServerBansCard = ({ name, url }) => {
  ...
  const loadBanlist = () => {
    ...
    fetchApi(url)
      .then((r) => r.json())
      .then((body) => {
        if (body.success) setBans(body.response);
        else setError(true);
      })
      .catch(() => setError(true))
      ...
      {bans?.unban.possible && (
        <div className="card-body pt-0">
          <div>
            ...
            Вы можете их снять за{' '}
            <b className="text-success">{ruPluralizeVimers(bans.unban.price)}</b>. Однако учтите,
            что после каждого снятия бана стоимость повышается. Соблюдайте{' '}
            <a href="https://vime.one/rules">правила</a>, чтобы это больше не повторялось!
          </div>

          <div className="text-end mt-3">
            <button className="btn btn-primary" onClick={() => setShowConfirmUnban(true)}>
              Снять бан
            </button>

            {bans.unban.price <= app.user.cash ? (
              <ConfirmModal
                show={showConfirmUnban}
                close={() => setShowConfirmUnban(false)}
                confirmText={'Снять бан'}
                onConfirm={unban}
                title={'Подтверждение'}>
                Вы действительно хотите купить снятие бана за{' '}
                <b className="text-success">{ruPluralizeVimers(bans.unban.price)}</b>?<br />
              ...
              <ConfirmModal
                ...
                У вас недостаточно вимеров для снятия бана.
                <br />
                Ваш баланс <b className="text-success">{ruPluralizeVimers(app.user.cash)}</b>
                <br />
                Стоимость снятия бана{' '}
                <b className="text-success">{ruPluralizeVimers(bans.unban.price)}</b>
              </ConfirmModal>
            )}
          </div>
        </div>
      ...
      <div className="card-table table-responsive">
        <table className="table table-payments">
          ...
          <tbody>
            {loading && !bans && (
              <tr>
                <td className="placeholder-glow" colSpan="5">
            ...
            {bans?.bans.map((b) => {
              ...
              switch (b.status) {
                ...
                case 'active_no_unban':
                  status = (
                    <OverlayTrigger overlay={<Tooltip>Без возможности снять бан</Tooltip>}>
