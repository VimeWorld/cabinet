    ...
    <table className="table">
      <thead className="bg-tertiary">
        <tr>
          ...
          <th width="120px" style={{ color: '#00BE00' }}>
            VIP
          </th>
          <th width="120px" style={{ color: '#00DADA' }}>
            Premium
          </th>
          <th width="120px" style={{ color: '#FFBA2D' }}>
            Holy
          </th>
          <th width="120px" style={{ color: '#E800D5' }}>
            Immortal
          </th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr>
          ...
          <td>
            <b style={{ color: '#00BE00' }}>[V]</b>
          </td>
          <td>
            <b style={{ color: '#00DADA' }}>[P]</b>
          </td>
          <td>
            <b style={{ color: '#FFBA2D' }}>[H]</b>
          </td>
          <td style={{ whiteSpace: 'nowrap' }}>
            <b style={{ color: '#E800D5' }}>
              [свой]
              <OverlayTrigger
                overlay={
                  <Tooltip>
              ...
              overlay={
                <Tooltip>
                  <div className="text-start">
                    <div>
                      <span style={{ color: '#000000' }} className="bg-secondary">
                        &0
                      </span>{' '}
                      - черный
                    </div>
                    <div>
                      <span style={{ color: '#0000bf' }} className="bg-secondary">
                        &1
                      </span>{' '}
                      - темно-синий
                    </div>
                    <div>
                      <span style={{ color: '#00be00' }}>&2</span> - зелёный
                    </div>
                    <div>
                      <span style={{ color: '#00bebe' }}>&3</span> - бирюзовый
                    </div>
                    <div>
                      <span style={{ color: '#be0000' }}>&4</span> - бордовый
                    </div>
                    <div>
                      <span style={{ color: '#be00be' }}>&5</span> - фиолетовый
                    </div>
                    <div>
                      <span style={{ color: '#D9A334' }}>&6</span> - оранжевый
                    </div>
                    <div>
                      <span style={{ color: '#bebebe' }}>&7</span> - серый
                    </div>
                    <div>
                      <span style={{ color: '#3f3f3f' }} className="bg-secondary">
                        &8
                      </span>{' '}
                      - темно-серый
                    </div>
                    <div>
                      <span style={{ color: '#3f3ffe' }}>&9</span> - синий
                    </div>
                    <div>
                      <span style={{ color: '#3ffe3f' }}>&a</span> - салатовый
                    </div>
                    <div>
                      <span style={{ color: '#3ffefe' }} className="bg-dark">
                        &b
                      </span>{' '}
                      - голубой
                    </div>
                    <div>
                      <span style={{ color: '#fe3f3f' }}>&c</span> - красный
                    </div>
                    <div>
                      <span style={{ color: '#fe3ffe' }}>&d</span> - розовый
                    </div>
                    <div>
                      <span style={{ color: '#fefe3f' }} className="bg-dark">
                        &e
                      </span>{' '}
                      - желтый
                    </div>
                    <div>
                      <span style={{ color: '#ffffff' }} className="bg-dark">
                        &f
                      </span>{' '}
                      - белый
                    </div>
...
const ExchangeCoins = ({ profile }) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <p>
        Сумма обменов: <b className="text-success">{ruPluralizeVimers(profile.donated)}</b>
      </p>

      <p>
        Работает система накопления. Все обмены суммируются и вы можете получить статус, когда
        количество ваших обменов достигнет определенной суммы.
      </p>
          ...
          <Form.Control
            {...register('vimers', {
              ...
              validate: (val) => {
                ...
                if (!Number.isInteger(val)) return 'Сумма в вимерах должна быть целой';
                if (val < 1) return 'Минимум 1 вимер';
      ...
      {errors.vimers?.message && (
        <div className="invalid-feedback d-block mt-n3">{errors.vimers.message}</div>
      )}
    ...
    <div className="card">
      ...
      <ProgressBar style={{ borderRadius: 0 }}>
        ...
        {segments.map((curr, idx) => {
          ...
          return (
            <OverlayTrigger key={idx} overlay={tooltip}>
              <ProgressBar
                isChild={true}
                style={{ backgroundColor: curr.barColor }}
                now={100 * (end - start)}
              />
            </OverlayTrigger>
          );
        ...
        <div className="row gy-4">
          <div className="col-lg-6 col-12">
            <p>
              Текущий статус:{' '}
              {ranks[profile.rank]
                ? ranks[profile.rank].rich || ranks[profile.rank].name
                : profile.rank}
...
const MinigamesDonatePage = () => {
  ...
  useEffect(() => {
    ...
    fetchApi('/server/minigames/profile')
      .then((r) => r.json())
      .then((body) => {
        if (body.success) setProfile(body.response);
    ...
    return (
      <div className="card">
        ...
        <div className="card-body">
          ...
          {!loading && !error && (
            <div className="text-center text-muted">Вы еще ни разу не заходили на MiniGames</div>
          )}
        </div>
      </div>
    );
