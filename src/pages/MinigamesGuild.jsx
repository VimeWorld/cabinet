  ...
  return (
    <div className="card">
      <div className="card-header">
        <h4 className="mb-0">
          Описание гильдии <span className="text-info">{guild.name}</span>
        </h4>
...
const AvatarCard = ({ guild }) => {
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h4 className="mb-0">
          Аватар гильдии <span className="text-info">{guild.name}</span>
        </h4>
        ...
        <form onSubmit={setAvatar}>
          <div className="d-block d-sm-flex d-md-block d-lg-flex">
            <div className="flex-shrink-1 d-flex align-items-center justify-content-center flex-column">
              <img
                src={guild.avatar || 'https://vimeworld.com/images/guild.png'}
                width="140"
                height="140"
                className="rounded"
              />
            ...
            <div className="flex-grow-1 ms-sm-4 ms-md-0 ms-lg-4 d-flex align-items-center justify-content-center flex-column">
              <div className="flex-grow-1 w-100 py-3">
                <label htmlFor="avatar_file" className="mb-1">
                  Аватар должен быть 140х140 в формате <code>.png</code> или <code>.jpg</code>
                </label>
                <input
                  ref={file}
...
const MinigamesGuildPage = () => {
  ...
  useEffect(() => {
    ...
    fetchApi('/server/minigames/guild')
      .then((r) => r.json())
      .then((body) => {
        if (body.success) setGuild(body.response);
        else setError(true);
      })
      .catch(() => setError(true))
      .finally(() => setLoading(false));
        ...
        <div className="card-body">
          ...
          {!loading && !error && (
            <div className="text-center text-muted">
              Эта страница доступна только лидеру гильдии
            </div>
          )}
        </div>
      </div>
